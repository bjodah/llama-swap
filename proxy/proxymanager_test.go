...
func TestProxyManager_APIModelsStatus(t *testing.T) {
	cfg := testConfigFromYAML(t, `
healthCheckTimeout: 15
logLevel: error
models:
...
`)
	...
	t.Run("GET /api/models returns all models", func(t *testing.T) {
	...
	t.Run("GET /api/models/ trailing slash also works", func(t *testing.T) {
		req := httptest.NewRequest("GET", "/api/models/", nil)
		w := CreateTestResponseRecorder()
		proxy.ServeHTTP(w, req)
	...
	t.Run("model state is reported", func(t *testing.T) {
		req := httptest.NewRequest("GET", "/api/models", nil)
		w := CreateTestResponseRecorder()
		proxy.ServeHTTP(w, req)
		assert.Equal(t, http.StatusOK, w.Code)
